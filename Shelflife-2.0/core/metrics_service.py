...
def is_consumed_after_expiry(item):
    if not item.is_consumed or not item.consumed_at:
        return False
    try:
        return item.consumed_at.date() > item.expiry_date
    except Exception:
...
def get_utilization_metrics(user_id):
    ...
    expired_items = [item for item in items if item.expiry_date < today and not item.is_consumed]
    consumed_items = [item for item in items if item.is_consumed]
    late_consumed_items = [item for item in consumed_items if is_consumed_after_expiry(item)]

    wasted_money = (
        sum(item_total_cost(item) for item in expired_items) +
        sum(item_total_cost(item) for item in late_consumed_items)
    )
