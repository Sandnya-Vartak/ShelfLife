...
function renderNotifications(notifications) {
    ...
    container.innerHTML = notifications.map((notification) => `
        <div class="notification-card ${getStatusClass(notification.status)} ${notification.is_read ? "read" : "unread"}">
            <div class="notification-card-header">
                <span class="status-pill ${getStatusClass(notification.status)}">${getStatusLabel(notification.status)}</span>
                <span class="notification-state">${notification.is_read ? "Read" : "Unread"}</span>
            </div>
...
document.getElementById("send-email-btn").addEventListener("click", async () => {
    try {
        const response = await apiRequest("/alerts/notifications/send-email", {
            method: "POST"
        });
        setNotificationMessage(response.message, "success");
        await initializeNotifications();
    } catch (error) {
        if (isAuthError(error)) {
            clearAuth();
