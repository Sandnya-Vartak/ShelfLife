...
def add_item():
    ...
    if notification and notification.status in {"expiring_critical", "expiring_soon", "expired"}:
...
def get_items():
    ...
    items = (
        Item.query
        .filter_by(user_id=user_id, is_consumed=False)
...
def get_pantry_items():
    ...
    items = (
        ...
        .filter_by(user_id=user_id, is_consumed=False)
        .filter(visible_inventory_filter())
        .order_by(Item.expiry_date.desc())
        .all()
    )
