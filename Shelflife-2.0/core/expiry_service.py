...
def get_auto_hide_expiry_cutoff():
    return date.today() - timedelta(days=AUTO_DELETE_EXPIRED_DAYS)


def visible_inventory_filter():
    expiry_cutoff = get_auto_hide_expiry_cutoff()
    return or_(Item.expiry_date > expiry_cutoff, Item.is_consumed.is_(True))
...
def cleanup_expired_items_for_user(user_id):
    # Expired items are now hidden from inventory views instead of being removed from storage.
    return 0
...
def refresh_notifications_for_user(user_id):
    items = (
        ...
        .filter(visible_inventory_filter())
        .all()
    )
    notifications = []
    for item in items:
...
def send_expiry_emails_for_user(user_id, include_meta=False):
    ...
    try:
        ...
        for notification in notifications:
            if notification.status not in {"expiring_soon", "expiring_critical", "expired"}:
                continue
...
def process_all_users_expiry_notifications():
    try:
        ...
        for user in users:
            try:
                deleted_items = cleanup_expired_items_for_user(user.id)
